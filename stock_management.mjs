...
import stocks from './stored_data/stock_products.json' assert { type: 'json' };
...
/* Display on terminal */
app.listen(port, () => {
    console.log(`The Server currently running at <http://localhost>:${chalk.green(port)}/stocks`);
    debug(`Server running at <http://localhost>:${chalk.green(port)}/stocks`);
});